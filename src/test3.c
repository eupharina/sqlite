...
static int SQLITE_TCLAPI btree_open(
  ...
  if( sqlite3StorePtr(interp, pBt, &pIdx) )
    return TCL_ERROR;
  sqlite3_snprintf(sizeof(zBuf), zBuf,"%x", pIdx);
...
static int SQLITE_TCLAPI btree_close(
  ...
  if( sqlite3TestTextToPtr(interp, argv[1], (void**)&pBt) )
    return TCL_ERROR;
...
static int SQLITE_TCLAPI btree_cursor(
  ...
  if( sqlite3StorePtr(interp, pCur, &pIdx) )
    return TCL_ERROR;
  sqlite3_snprintf(sizeof(zBuf), zBuf,"%x", pIdx);
  Tcl_AppendResult(interp, zBuf, 0);
  return SQLITE_OK;
...
static int SQLITE_TCLAPI btree_close_cursor(
  ...
  if( sqlite3TestTextToPtr(interp, argv[1], (void**)&pCur) )
    return TCL_ERROR;
...
static int SQLITE_TCLAPI btree_insert(
  ...
  if( sqlite3TestTextToPtr(interp, Tcl_GetString(objv[1]), (void**)&pCur) )
    return TCL_ERROR;
